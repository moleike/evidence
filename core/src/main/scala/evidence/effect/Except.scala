...
object Except:
  ...
  private[evidence] final class Ops[A](val dummy: Boolean = true)
    ...
    def handleError[E, Ans](
        f: A => Ans
    ): Eff[Except[A, *, *] :* E, Ans] => Eff[E, Ans] = handleErrorWith(
      f(_).pure
    )
    ...
    def toOption[E, Ans]
        : Eff[Except[A, *, *] :* E, Ans] => Eff[E, Option[Ans]] =
      Eff.handlerRet(
        (b: Ans) => b.some,
        new Except[A, E, Option[Ans]] {
    ...
    def toEither[E, Ans]
        : Eff[Except[A, *, *] :* E, Ans] => Eff[E, Either[A, Ans]] =
      Eff.handlerRet(
        (b: Ans) => Right(b),
        new Except[A, E, Either[A, Ans]] {
